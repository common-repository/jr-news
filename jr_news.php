...
/*
Plugin Name: JR News
Plugin URI: http://www.jakeruston.co.uk/2009/11/wordpress-plugin-jr-news/
Description: This plugin allows you to show the latest News updates as a widget.
Version: 1.4.5
Author: Jake Ruston
...
function jr_news_add_pages() {
    add_options_page('JR News', 'JR News', 'administrator', 'jr_news', 'jr_news_options_page');
}
...
if (!function_exists("jr_news_refresh")) {
function jr_news_refresh() {
update_option("jr_submitted_news", "0");
}
}

register_activation_hook(__FILE__,'news_choice');

function news_choice () {
if (get_option("jr_news_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_news";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_news", "1");
wp_schedule_single_event(time()+172800, 'jr_news_refresh'); 
} else {
...
update_option("jr_news_links_choice", $content);
}
}

if (get_option("jr_news_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_news_link_personal", $content);
}
}

// jr_news_options_page() displays the page content for the Test Options submenu
function jr_news_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR News Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
echo $before_title.$news_header.$after_title.$before_widget; 

$doc = new DOMDocument();

$doc->load('http://rss.news.yahoo.com/rss/'.$news_type) or die("Error!");
...
$news = str_replace("(AP)", "", $news);

echo "<li style='color:#".$twittercolour."'><a href='".$newslink."' rel='nofollow'>".$news."</a></li>";
if($i++ >= $news_number) break;

}

if ($plugin_support2=="Yes" || $plugin_support2=="") {
$linkper=utf8_decode(get_option('jr_news_link_personal'));

if (get_option("jr_news_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_news_links_choice'));
...
update_option("jr_news_links_choice", $new);
update_option("jr_news_link_newcheck", "444");
}

if (get_option("jr_submitted_news")=="0") {
$pname="jr_news";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_news", "1");
...
wp_schedule_single_event(time()+172800, 'jr_news_refresh'); 
} else if (get_option("jr_submitted_news")=="") {
$pname="jr_news";
$url=get_bloginfo('url');
$current=get_option("jr_news_links_choice");
...
echo "<p style='color: #".$newscolor."; font-size:x-small'>News Plugin created by ".$linkper." - ".stripslashes(get_option('jr_news_links_choice'))."</p>";
}
...
register_sidebar_widget("JR News", "show_news");
}
